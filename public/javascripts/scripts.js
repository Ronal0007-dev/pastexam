document.addEventListener('DOMContentLoaded', function () {
  ...
  const dropdownParents = document.querySelectorAll('.nav-item.has-dropdown > a');
  ...
  // Make dropdowns clickable on mobile (toggle parent li.open)
...
document.addEventListener('DOMContentLoaded', () => {
  const toggle = document.querySelector('.nav-toggle');
  const navMenu = document.querySelector('.nav-menu');
  const dropdownAnchors = document.querySelectorAll('.nav-item.has-dropdown > a');
  ...
  function closeMenu() {
    ...
    navMenu.classList.remove('open');
    toggle.setAttribute('aria-expanded', 'false');
    document.querySelectorAll('.nav-item.has-dropdown.open').forEach(li => li.classList.remove('open'));
  }
  ...
  // Close on outside click (only for small screens)
  document.addEventListener('click', (e) => {
    if (window.innerWidth <= 900 && !navMenu.contains(e.target) && !toggle.contains(e.target)) {
      closeMenu();
    }
  });

  // Toggle mobile dropdowns by clicking the parent anchor (prevent navigation)
  dropdownAnchors.forEach(anchor => {
    anchor.addEventListener('click', (e) => {
      if (window.innerWidth <= 900) {
        ...
        // optionally set aria-expanded on the anchor
  ...
  // Optional: close menu when a regular nav link (non-dropdown) is clicked on mobile
  document.querySelectorAll('.nav-list a:not(.has-dropdown a)').forEach(a => {
    a.addEventListener('click', () => {
